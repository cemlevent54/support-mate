...
const activeSessions = new Map<string, { token: string; timestamp: number }>();
...
class JWTUtils {
  ...
  static generateAccessToken(payload: UserPayload, expiresIn: string = '24h') {
    logger.debug(`Generating access token - User ID: ${payload.id}, Email: ${payload.email}`);
    const expiresInSec = parseDuration(expiresIn);
    const token = jwt.sign(payload, JWT_SECRET as string, { expiresIn: expiresInSec });
  ...
  static generateRefreshToken(payload: UserPayload) {
    logger.debug(`Generating refresh token - User ID: ${payload.id}, Email: ${payload.email}`);
    const token = jwt.sign(payload, JWT_REFRESH_SECRET as string, { expiresIn: '7d' });
  ...
  static generateJWT(user: UserPayload) {
  ...
  static generatePasswordResetToken(user: UserPayload) {
